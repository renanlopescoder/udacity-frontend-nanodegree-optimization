...
var resizePizzas = function(size) {
  ...
  // Adjusts made inside the for loop removing variables and optimizing the process
  // Getting the first size and resizing all pizzas
...
function updatePositions() {
  ...
  //Changed to select mobile pizzas by class name instead of querySelectorAll

  var items = document.getElementsByClassName('mover');
  
  //Moved phase calculation with his own for loop
  var phase = [];
  var calc = document.body.scrollTop / 1250;
  for (var index = 0; index < 5; index++) {
    phase.push(Math.sin(calc + (index % 5)));
  }

  //The pizza style are changed accessing a relevant element of the "phase" array
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var calcPizzas = (screen.height / s) * cols;
  // Reduced quantity of pizzas inside the screen
  for (var i = 0; i < calcPizzas; i++) {
